...
func TestParserSimple(t *testing.T) {
	...
	data := []TestItem{
		...
		{`(RAW_TEXT CONTAINS "?")`, `P{(RAW_TEXT CONTAINS "?")}`},
		{`(RAW_TEXT CONTAINS ?)`, `P{(RAW_TEXT CONTAINS ?)}`},
		{` ( ( RAW_TEXT CONTAINS "hello " ) ) `, `P{P{(RAW_TEXT CONTAINS "hello ")}}`},
		{` ( RECORD.Name.Actors.[].Name CONTAINS "Christian" ) `, `P{(RECORD.Name.Actors.[].Name CONTAINS "Christian")}`},
		{`(RECORD.body CONTAINS "FEDS")`, `P{(RECORD.body CONTAINS "FEDS")}`},
		{`(RECORD.body CONTAINS FHS("test", cs = true, dist = 10, WIDTH = 100))`, `P{(RECORD.body CONTAINS "test")[fhs,d=10,w=100,cs=true]}`},
		{`(RECORD.body CONTAINS FEDS("test", cs= FALSE ,  DIST =10, WIDTH=100))`, `P{(RECORD.body CONTAINS "test")[feds,d=10,w=100]}`},
		{`(RECORD.body CONTAINS FEDS("test", ,, DIST =0, WIDTH=10))`, `P{(RECORD.body CONTAINS "test")[es,w=10]}`},
		{`(RAW_TEXT CONTAINS DATE(MM/DD/YY > 02/28/12))`, `P{(RAW_TEXT CONTAINS DATE(MM/DD/YY>02/28/12))[ds]}`},
		{`(RAW_TEXT CONTAINS DATE(02/28/12 < MM/DD/YY < 01/19/15))`, `P{(RAW_TEXT CONTAINS DATE(02/28/12<MM/DD/YY<01/19/15))[ds]}`},
		{`(RAW_TEXT CONTAINS TIME(HH:MM:SS > 09:15:00))`, `P{(RAW_TEXT CONTAINS TIME(HH:MM:SS>09:15:00))[ts]}`},
		{`(RAW_TEXT CONTAINS TIME(11:15:00 < HH:MM:SS < 13:15:00))`, `P{(RAW_TEXT CONTAINS TIME(11:15:00<HH:MM:SS<13:15:00))[ts]}`},
		{`(RECORD.price CONTAINS CURRENCY("$450" < CUR < "$10,100.50", "$", ",", "."))`, `P{(RECORD.price CONTAINS CURRENCY("$450"<CUR<"$10,100.50","$",",","."))[ns]}`},
		{`(RECORD.body CONTAINS REGEX("\w+", CASELESS))`, `P{(RECORD.body CONTAINS REGEX("\w+",CASELESS))[rs]}`},
		{`(RAW_TEXT CONTAINS "100")`, `P{(RAW_TEXT CONTAINS "100")}`},
		{`((RAW_TEXT CONTAINS "100"))`, `P{P{(RAW_TEXT CONTAINS "100")}}`},
		{`(RAW_TEXT CONTAINS "DATE()")`, `P{(RAW_TEXT CONTAINS "DATE()")}`},
		{`(RAW_TEXT CONTAINS "TIME()")`, `P{(RAW_TEXT CONTAINS "TIME()")}`},
		{`(RAW_TEXT CONTAINS "NUMBER()")`, `P{(RAW_TEXT CONTAINS "NUMBER()")}`},
		{`(RAW_TEXT CONTAINS "CURRENCY()")`, `P{(RAW_TEXT CONTAINS "CURRENCY()")}`},
		{`(RAW_TEXT CONTAINS "REGEX()")`, `P{(RAW_TEXT CONTAINS "REGEX()")}`},
	}
